...
export class DropdownsComponent extends AbstractSectionComponent
  implements OnInit {
  // public members
  public dropdownModel: Array<any> = [];
  public dropdownSelectionText = '';
  ...
  ngOnInit() {
    this.addSubscription = this.dropdown$.subscribe(model => {
      ...
      } else if (model.filter(item => item.selected).length === 1) {
        this.dropdownSelectionText = '1 Category Selected';
      } else if (model.filter(item => item.selected).length === model.length) {
        this.dropdownSelectionText = `All Categories Selected`;
      } else {
        this.dropdownSelectionText = `${model.filter(item => item.selected)
  ...
  toggleItem(itemId: number) {
    this.dropdown$
      .take(1)
      .subscribe(state => this.dropdown$.next(update(state, Number(itemId))));
    ...
    function update(state, id) {
      return state
        ...
        .reduce((items, item, index, orig_collection) => {
          if (item.type === 'all') {
            const all_items = orig_collection
                .slice(1)
                .every(itm => itm.selected),
              some_items = orig_collection.slice(1).some(itm => itm.selected);
            if (some_items) {
              item.selected = false;
              item.partial = true;
            }
